...
def slide(kind: str, title: str, body: str, *, sub: str = "", stamp: str = "",
          foot: str = "", fig: str = "", cls: str = "") -> None:
    """Append one slide. `kind` sets the rail colour and the eyebrow, and is a real type."""
    n = len(SLIDES) + 1
    head = ""
    if title:
        head = (f'<header><div class="eyebrow"><span class="kind">{SECTION[kind]}</span>'
                f'<span>{title_eyebrow(kind)}</span>'
    ...
    SLIDES.append(
        ...
        f'<footer class="foot"><span>{foot or "CognitionBioChem"}</span>'
...
def title_eyebrow(kind: str) -> str:
    return _EYEBROW[kind]


def figslide(kind, title, sub, name, caption, stamp="", foot=""):
    slide(kind, title, f'<div class="figwrap"><img src="{img(name)}" alt="{caption}">'
...
HTML = (
    '<title>CognitionBioChem</title>\n'
    '<meta name="description" content="Conference deck: a structural pharmacology workbench '
    'that reports a negative result.">\n'
    ...
    '<div class="hint" aria-hidden="true">&larr; &rarr; to navigate</div>\n'
    f"<script>{JS}</script>\n"
)
...
# The PDF is written by the same command that writes the HTML, so the two cannot drift.
# A deck exported once by hand goes stale the first time a study is re-run, and a stale
# slide is exactly the failure this repository exists to prevent.
    ...
    with socketserver.TCPServer(("127.0.0.1", 0), Handler) as srv:
        ...
        r = subprocess.run(
            [str(CHROME), "--headless=new", "--disable-gpu", "--window-size=1440,900",
             ...
             f"--print-to-pdf={PDF}",
             f"http://127.0.0.1:{port}/{OUT.name}"],
