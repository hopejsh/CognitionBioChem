#!/usr/bin/env python3
"""Build docs/CognitionBioChem_Deck.pptx -- the deck as an editable PowerPoint file.
...
SRC = REPO / "docs" / "CognitionBioChem_Deck.html"
OUT = REPO / "docs" / "CognitionBioChem_Deck.pptx"
...
def est_height(runs, width_px: float, size_px: float, face: str = FONT_BODY,
...
def put(para, runs, *, size_px, face=FONT_BODY, color=INK, bold=False, italic=False,
...
def build() -> int:
    if not SRC.exists():
        raise FileNotFoundError(f"{SRC} is missing; run platform/build_deck.py first")
    ...
    prs.core_properties.comments = (
        f"Generated by platform/build_pptx.py from docs/CognitionBioChem_Deck.html at "
        f"{git_sha()}. Every number on these slides is read from a study artefact at build "
        f"time -- re-run the generators rather than retyping one.")
    _assert_nothing_dropped(prs, SRC.read_text())
    prs.save(OUT)
