...
def build() -> int:
    ...
    para(doc,
         ...
         f"막아 주지 못한다. 슬레이트 #12가 그 해석을 등록된 방식으로 검정했고 그것은 "
         f"살아남지 못했다. 이 영가설은 천연 서열을 자기 순열과 짝들의 집합 단위로 "
         f"분리해 낼 뿐, 짝 하나에 대해서는 아무 판정도 정당화하지 않는다. 같은 방식으로 "
         f"읽히는 스크리닝에 대해서도 아무 일도 하지 않으며, "
